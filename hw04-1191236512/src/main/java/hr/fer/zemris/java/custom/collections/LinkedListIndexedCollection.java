...
public class LinkedListIndexedCollection<T> implements List<T> {
	...
	private static class ListElementsGetter<S> implements ElementsGetter<S> {
		...
		public ListElementsGetter(LinkedListIndexedCollection<S> col) {
			collection = col;
		...
		public S getNextElement() {
			...
			S currentObject = currentNode.value;
	...
	public void insert(T value, int position) {
		...
		if(position == 0) {
			...
			ListNode<T> current = find(position);
